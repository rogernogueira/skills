...
`<skill-dir>/scripts/validate-lesson.mjs` cobre a estrutura do painel,
`<skill-dir>/scripts/test-completion.mjs` cobre o portão de conclusão e
`<skill-dir>/scripts/test-scorm.mjs` cobre persistência, retomada e bloqueio por falha de
commit. Os itens manuais — leitor de tela, zoom, impressão, LMS real — só podem ser declarados
como executados se tiverem sido executados.
